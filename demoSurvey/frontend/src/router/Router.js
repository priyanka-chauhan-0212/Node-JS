...
import { DefaultRoute, Routes } from "./routes/index";
...
// import ResetPasswordCover from '../views/authPages/ResetPasswordCover';

import { useCookies } from "react-cookie";
import { GET_USER_PROFILE_API_PATH } from "../ApiRoutes/ApiRoutes";
...
const Router = () => {
  ...
  console.log("userData from router page", userData);
      ...
      jwt.verify(token, "secret_key", (err, data) => {
        if (data) {
          if (!userData || userData == null) {
            axiosInstance
              .post(GET_USER_PROFILE_API_PATH, data)
              .then((response) => {
          ...
          >
            <Switch>
              {LayoutRoutes.map((route) => {
                let name = "";
                route.isPublicRoute && route.isPublicRoute
                  ? (name = "Route")
                  : (name = "PrivateRoute");
